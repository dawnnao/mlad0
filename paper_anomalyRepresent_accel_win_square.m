...
saveName = ['accel_' date '-' num2str(h,'%02d') '_ch' tidyName(abbr(channel)) '_square.emf'];
saveas(gcf, [folderName '/' saveName]);
saveName = ['accel_' date '-' num2str(h,'%02d') '_ch' tidyName(abbr(channel)) '_square.tif'];
saveas(gcf, [folderName '/' saveName]);
saveName = ['accel_' date '-' num2str(h,'%02d') '_ch' tidyName(abbr(channel)) '_square.eps'];
saveas(gcf, [folderName '/' saveName]);