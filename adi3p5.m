function sensor = adi3p5(pathRoot, sensorNum, dateStart, dateEnd, sensorTrainRatio, sensorPSize, step)
% DESCRIPTION:
...
%% common variables
sensor.label.name = {'1-normal','2-outlier','3-minor','4-missing','5-trend','6-drift','7-bias','8-cutoff'};
...
while goNext == 0
    ...
    for s = sensor.num
        t(2) = tic;
        sensor.label.manual{s} = zeros(labelNum,hourTotal);
        % manually label
        ...
        while n <= sensor.trainSetSize(s)
            sensor.label.manual{s}(:,sensor.random(n)) = zeros(labelNum,1);  % initialize for re-label if necessary
            ...
            if classify <= labelNum && classify >= 1
